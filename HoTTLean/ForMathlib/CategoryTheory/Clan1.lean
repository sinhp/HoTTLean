...
def pullbackForgetTwoSquare {T : Type u} [Category.{v} T] [HasFiniteWidePullbacks T]
    {R : MorphismProperty T} {X Y : T} (f : X ⟶ Y) [R.IsStableUnderBaseChangeAlong f] :
...
example {T : Type u} [Category.{max u v} T]
    (R : MorphismProperty T) [R.HasPullbacks] [R.IsStableUnderBaseChange]
    {X Y Z : T} (f : X ⟶ Y) (t : Z ⟶ Y)
    [HasPullbacksAlong f] :
    HasPullbacksAlong (pullback.fst t f) :=
  inferInstance

def IsPushforward.ofYonedaIsPushforward {C : Type u} [Category.{u} C]
    {S S' : C} (f : S ⟶ S') (X : Over S) (Y : Over S') [HasPullbacksAlong f]
    (isPushforward : IsPushforward ym(f) ((CategoryTheory.Over.post yoneda).obj X)
      ((CategoryTheory.Over.post yoneda).obj Y)) :
    IsPushforward f X Y where
  homEquiv {A} := by
    refine (Yoneda.fullyFaithful.over S').homEquiv.trans ?_
    refine (isPushforward.homEquiv).trans ?_
    refine Equiv.trans ?_ (Yoneda.fullyFaithful.over S).homEquiv.symm
    refine Iso.homCongr ?_ (Iso.refl _)
    exact CategoryTheory.Over.isoMk (PreservesPullback.iso yoneda A.hom f).symm
  homEquiv_comp {A A'} g h := by
    -- apply (Yoneda.fullyFaithful.over S).map_injective
    -- ext : 1
    -- simp only [Functor.op_obj, Functor.id_obj, Functor.comp_obj,
    --   yoneda_obj_obj, Functor.const_obj_obj, Equiv.trans_apply, Iso.homCongr_apply, Iso.refl_hom,
    --   comp_id, Functor.comp_map, Functor.op_map, Quiver.Hom.unop_op,
    --   yoneda_obj_map, Over.comp_left, Functor.map_comp, Functor.FullyFaithful.homEquiv_symm_apply]
    sorry

/-- The constructed pushforward functor for `pullback.fst t f : pullback t f ⟶ Z`,
given `f : X ⟶ Y` has pushforwards and `R`-map `t : Z ⟶ Y`. -/
def pushforwardPullbackFst {T : Type u} [Category.{v} T]
    (R : MorphismProperty T) [R.HasPullbacks] [R.IsStableUnderBaseChange]
    [R.IsStableUnderComposition]
    {X Y Z : T} (f : X ⟶ Y) {t : Z ⟶ Y} (rt : R t)
    [HasPullbacksAlong f] [R.HasPushforwardsAlong f] [R.IsStableUnderPushforwardsAlong f] :
    R.Over ⊤ (pullback t f) ⥤ R.Over ⊤ Z :=
  Over.map ⊤ (f := pullback.snd t f) (R.of_isPullback (IsPullback.of_hasPullback t f) rt) ⋙
  R.pushforward f ⋙ Over.pullback R ⊤ t

instance hasPushforwardsAlong_pullback_fst {T : Type u} [Category.{max u v} T]
    (R : MorphismProperty T) [R.HasPullbacks] [R.IsStableUnderBaseChange]
    {X Y Z : T} (f : X ⟶ Y) (t : Z ⟶ Y)
    [HasPullbacksAlong f] [R.HasPushforwardsAlong f] [R.IsStableUnderPushforwardsAlong f] :
    R.HasPushforwardsAlong (pullback.fst t f) where
  hasPushforward {W} h rh := {
    has_representation := ⟨sorry, ⟨sorry⟩⟩ }

instance isStableUnderPushforwardsAlong_pullback_fst {T : Type u} [Category.{max u v} T]
    (R : MorphismProperty T) [R.HasPullbacks] [R.IsStableUnderBaseChange]
    {X Y Z : T} (f : X ⟶ Y) {t : Z ⟶ Y} (rt : R t)
    [HasPullbacksAlong f] [R.HasPushforwardsAlong f] [R.IsStableUnderPushforwardsAlong f] :
    R.IsStableUnderPushforwardsAlong (pullback.fst t f) :=
  sorry

def extendedFibration_pushforward {T : Type u} [Category.{max u v} T]
    (R : MorphismProperty T) [R.HasPullbacks] [R.IsStableUnderBaseChange]
    {X Y : T} (f : X ⟶ Y)
    [HasPullbacksAlong f] [R.HasPushforwardsAlong f] [R.IsStableUnderPushforwardsAlong f]
    {W : Psh T} (h : W ⟶ y(X)) (rh : R.RepresentableFibrantChosenPullbacks h) :
    R.RepresentableFibrantChosenPullbacks ((ExponentiableMorphism.pushforward ym(f)).obj
      (CategoryTheory.Over.mk h)).hom where
  ext {Γ} t := by
    dsimp at t
    -- let E := rf.ext t
    -- let d := rf.disp t
...
lemma extendedFibration_pushforward_yoneda {T : Type u} [Category.{max u v} T]
    (R : MorphismProperty T) [R.HasPullbacks] [R.IsStableUnderBaseChange] {X Y : T} (f : X ⟶ Y)
    [HasPullbacksAlong f] [R.HasPushforwardsAlong f] [R.IsStableUnderPushforwardsAlong f]
    {W : Psh T} (h : W ⟶ y(X)) (rh : R.ExtendedFibration h) :
    R.ExtendedFibration ((ExponentiableMorphism.pushforward ym(f)).obj
      (CategoryTheory.Over.mk h)).hom :=
  ⟨ sorry ⟩

instance {T : Type u} [Category.{max u v} T] (R : MorphismProperty T)
    [R.HasPullbacks] [R.IsStableUnderBaseChange] {X Y : T} (f : X ⟶ Y)
    [HasPullbacksAlong f] [R.HasPushforwardsAlong f] [R.IsStableUnderPushforwardsAlong f] :
    (ExtendedFibration R).IsStableUnderPushforwardsAlong ym(f) :=
  IsStableUnderPushforwardsAlong.of_respectsIso _ _
  (fun h _ => (ExponentiableMorphism.pushforward ym(f)).obj (CategoryTheory.Over.mk h))
  (fun h rh => extendedFibration_pushforward_yoneda R f h rh)
  (fun h _ => ExponentiableMorphism.pullbackRepresentableByPushforward ym(f) (.mk h))

theorem pushforwardPullbackTwoSquare_isIso_of_exponentiableMorphism
    {T : Type u} [Category.{v} T] [HasFiniteWidePullbacks T] (R : MorphismProperty T)
    [R.HasPullbacks] [R.IsStableUnderBaseChange]
    {X Y Z W : T} (h : X ⟶ Z) (f : X ⟶ Y) (g : Z ⟶ W) (k : Y ⟶ W)
    [ExponentiableMorphism f] [ExponentiableMorphism g]
    [R.IsStableUnderPushforwardsAlong f] [R.IsStableUnderPushforwardsAlong g]
    (pb : IsPullback h f g k) :
    IsIso (pushforwardPullbackTwoSquare (R := R) h f g k pb.w) := by
  let α : (R.pushforward g ⋙ Over.pullback R ⊤ k) ⋙
    Over.forget R ⊤ Y ⟶
    (Over.pullback R ⊤ h ⋙ R.pushforward f) ⋙
    Over.forget R ⊤ Y := sorry
  -- TODO: define α as the following composition. All should be either x.hom for some iso x or
    -- a morphism such that IsIso x
  -- (R.ExtendedFibration.pushforward g ⋙ Over.pullback R.ExtendedFibration ⊤ k) ⋙
  -- Over.forget R.ExtendedFibration ⊤ Y
  -- ≅ R.ExtendedFibration.pushforward g ⋙ Over.pullback R.ExtendedFibration ⊤ k ⋙ Over.forget R.ExtendedFibration ⊤ Y
  -- ≅ R.ExtendedFibration.pushforward g ⋙ Over.forget R.ExtendedFibration ⊤ W ⋙ Over.pullback (ExtendedFibration R) ⊤ ym(k)
...
theorem pushforwardPullbackTwoSquare_isIso_extendedFibration {T : Type u} [Category.{max u v} T]
    ...
    {X Y Z W : T} (h : X ⟶ Z) (f : X ⟶ Y) (g : Z ⟶ W) (k : Y ⟶ W)
    [HasPullbacksAlong f] [HasPullbacksAlong g]
    [R.HasPushforwardsAlong f] [R.HasPushforwardsAlong g]
    [R.IsStableUnderPushforwardsAlong f] [R.IsStableUnderPushforwardsAlong g]
    (pb : IsPullback h f g k) :
    IsIso (pushforwardPullbackTwoSquare (R := ExtendedFibration R) ym(h) ym(f) ym(g) ym(k)
      (by simp [← Functor.map_comp, pb.w])) :=
  pushforwardPullbackTwoSquare_isIso_of_exponentiableMorphism
    (ExtendedFibration R) ym(h) ym(f) ym(g) ym(k) (Functor.map_isPullback _ pb)
