...
import HoTTLean.ForMathlib.CategoryTheory.Quotient
...
@[simp]
lemma of.map_id (X : C) : (Quotient.functor (FreeGroupoid.homRel C)).map ((Groupoid.Free.of C).map (𝟙 X)) =
    𝟙 _:= by
...
def free : Cat.{u,u} ⥤ Grpd.{u,u} where
  ...
  map {C D} F := map F
  map_id C := by
    simp [Grpd.id_eq_id, ← map_id]
    rfl
  map_comp F G := by
    simp [Grpd.comp_eq_comp, ← map_comp]
    rfl
