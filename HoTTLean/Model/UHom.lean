...
def hom (s : UHomSeq R) (i j : Nat) (ij : i < j := by omega)
...
@[reassoc]
theorem mk_comp_cartesianNatTrans {i j ij jlen} {Γ X} (A : (Γ) ⟶ s[i].Ty)
    (B : (s[i].ext A) ⟶ X) :
    PtpEquiv.mk s[i] A B ≫ (s.cartesianNatTrans i j).app X =
    PtpEquiv.mk s[j] (A ≫ (s.homOfLe i j).mapTy)
    ((substCons _ (s[j].disp _) _ (s.unliftVar i j ij jlen A rfl) (by simp)) ≫ B) := by
...
theorem mk_comp_cartesianNatTransTm {i0 i1 j0 j1 ii ilen jj jlen}
    ...
    : PtpEquiv.mk s[i0] A B ≫ s.cartesianNatTransTm i0 i1 j0 j1 ii ilen jj jlen =
      PtpEquiv.mk s[i1] (A ≫ (s.homOfLe i0 i1).mapTy)
        ((substCons _ (s[i1].disp _) _ (s.unliftVar i0 i1 ii ilen A rfl) (by simp))
...
theorem hom_comp_trans (s : UHomSeq R) (i j k : Nat) (ij : i < j) (jk : j < k)
    (klen : k < s.length + 1) :
